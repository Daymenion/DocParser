...
class DotsOCRParser:
    ...
    def __init__(self, 
        ...
        self.use_hf = use_hf
        # Allow environment override for model path
    ...
    def _load_hf_model(self):
        """Load HF model with robust attention and dtype fallback.
        Prefers flash_attention_2, falls back to sdpa or eager if unavailable.
        """
        import torch
        ...
        # dtype preference: bfloat16 if cuda and supported, else float16, else float32
        torch_dtype = torch.bfloat16
        if not torch.cuda.is_available():
            torch_dtype = torch.float32
        else:
            # Some GPUs may not support bfloat16 well; allow fallback to float16
            try:
        ...
        for attn_impl in attn_impl_candidates:
            try:
                ...
                print(f"Loaded HF model from {self.model_path} with attn_impl={attn_impl}, dtype={torch_dtype}")
                break
            except Exception as e:
                last_error = e
                print(f"Failed to load model with attn_impl={attn_impl}: {e}")
                self.model = None
        if self.model is None:
            raise RuntimeError(f"Failed to load model from {self.model_path}: {last_error}")
